...
const DetailPage = () => {
    ...
    useEffect(() => {
        ...
        const alreadyInstalled = existingData.some(p => p.id === AppsData?.id);
        if (alreadyInstalled) {
            setIsInstalled(true);
        }
    }, [AppsData?.id]);
    const handleInstallButton = () => {
        const existingData = JSON.parse(localStorage.getItem('Instal')) || [];
        const isDuplicate = existingData.some(p => p.id === AppsData.id)
        if (!isDuplicate) {
            const updatedData = [...existingData, AppsData];
            localStorage.setItem('Instal', JSON.stringify(updatedData));
